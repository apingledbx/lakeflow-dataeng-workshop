...
# MAGIC 7. `USE CATALOG` + `CREATE SCHEMA` on `de_workshop` to group `account users` — attendees
# MAGIC    self-serve their own schema `de_workshop.<short_name>` via `misc/create_my_schema.py`
# MAGIC    (replaces the old Vocareum pre-provisioning)
# MAGIC
...
# `de_workshop` is the attendee catalog. This notebook creates it if missing; the grants in
# Step 3 then let each attendee self-serve their own schema de_workshop.<short_name>.
spark.sql(f"CREATE CATALOG IF NOT EXISTS {CATALOG} COMMENT 'Attendee catalog for the Lakeflow DataEng workshop; per-short_name schemas live here'")
...
# Attendee self-service on the attendee catalog. Replaces the old Vocareum model
# (where each attendee's schema was pre-provisioned externally). Here every attendee
# gets USE CATALOG + CREATE SCHEMA on `de_workshop` and creates their own schema
# `de_workshop.<short_name>` by running misc/create_my_schema.py once. No attendee list needed.
...
print(f"Grants applied: USE CATALOG + CREATE SCHEMA on {CATALOG}  →  `account users`  (attendees self-serve their own schema)")
...
if ZEROBUS_REGION and not _REGION_RE.fullmatch(ZEROBUS_REGION):
    raise ValueError("Set 'zerobus_region' to a region-like value such as 'us-west-2' or 'eastus', or blank to skip Part B.")
...
# Create the zerobus schema. If a managed location was provided, pin the schema to it so
# every table underneath lands on real cloud storage (overrides the catalog default). A
# schema-level managed location makes the CREATE SCHEMA IF NOT EXISTS a no-op on re-run.
if ZEROBUS_MANAGED_LOCATION:
    spark.sql(
        f"CREATE SCHEMA IF NOT EXISTS {OPS_CATALOG}.zerobus "
        f"MANAGED LOCATION '{ZEROBUS_MANAGED_LOCATION}' "
        f"COMMENT 'Zerobus ingest targets (managed location on real cloud storage)'"
    )
else:
